...
This paper investigates whether purely local plasticity rules (STDP and homeostatic plasticity), combined with a physical guidance training protocol, can enable a chaotic spiking neural network to acquire a cross-modal sensorimotor mapping (visual shape → vocalization trajectory). We ask how close a local rule can come to global optimization methods, rather than claiming equivalence.
...
The main finding is that STDP with physical guidance achieves 0.865 correlation, approaching the 0.873 of offline ridge regression and 0.875 of online RLS/FORCE. The gap is small (about 1%), and the difference does not reach statistical significance at p < 0.05 with 10 seeds. However, as DeepSeek notes, "p > 0.05 is not equivalence" — the moderate effect sizes (Cohen's d ≈ −0.5 to −0.6) indicate that STDP is somewhat below the global baselines, and the non-significant result may partly reflect low statistical power.

From an information-theoretic perspective, the more interesting observation is that a local rule using one scalar per synapse can come this close to a global method using a 1000×1000 matrix. The 1% performance gap comes with a many-orders-of-magnitude reduction in per-synapse information requirements.
...
1. **Scalability.** Performance collapses above 1000 neurons without parameter retuning. The method currently relies on a carefully tuned operating point (spectral radius ≈ 5.0). Biological systems maintain stability across scale through mechanisms absent from our model (e.g., structural plasticity, neuromodulation, hierarchical organization). This work should be understood as a **proof of concept** demonstrating that local plasticity can approach global optimization performance in a controlled setting — not as a general-purpose algorithm ready for large-scale deployment.
...
In a small-scale chaotic spiking network (1000 neurons, 3 shapes, 8 motor parameters), local STDP weight updates paired with global physical guidance achieve performance that shows no statistically significant difference from offline ridge regression and online RLS/FORCE (0.865 vs 0.873/0.875), though effect sizes indicate STDP is slightly lower. This suggests that local weight-update rules, when paired with appropriate guidance, can approach the performance of global linear readout methods. However, statistical equivalence is not established, and general scalability to larger networks and more complex tasks remains unknown. The primary contribution is a clear proof-of-concept demonstration that a local plasticity rule can come surprisingly close to global optimization on a sensorimotor task — using orders of magnitude less information per synapse.
